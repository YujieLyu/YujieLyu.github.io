        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {data.achievements.map((achievement) => {
            ...
            const wrapperProps = achievement.url
              ? { href: achievement.url, target: '_blank' as const, rel: 'noopener noreferrer' }
              : {}
            return (
              <Wrapper
                key={achievement.title}
                {...wrapperProps}
                className={`bg-white dark:bg-[#242526] rounded-xl overflow-hidden shadow-md border border-gray-200 dark:border-gray-700 flex flex-col transition-transform hover:scale-[1.02]${achievement.url ? ' cursor-pointer' : ''}`}
              >
